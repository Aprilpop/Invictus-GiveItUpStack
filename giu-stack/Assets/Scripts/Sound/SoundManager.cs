...
public class SoundManager : MonoBehaviour {
	...
	SoundObject RadioSoundObject = null;
	SfxArrayEnum radioCurrent;
	RadioState radioState;

	public static void PlayRadio(SfxArrayEnum value, bool immidiate = true)
	{
		if (instance.radioCurrent != SfxArrayEnum.unknown)
			return;

		instance.radioCurrent = value;

	}

	void SetupRadioSound(SfxArrayEnum value)
	{
		string sT = Enum.GetName(typeof(SfxArrayEnum), value);
		if (sT == "")
		{
		...
		else
		{
			...
			if (instance.RadioSoundObject == null)
				instance.RadioSoundObject = instance.GetSoundObject("Radio");

			SfxContainer container = instance.LookupClip(sT);

			if (container != null && container.sounds.Length > 0)
			{
				instance.RadioSoundObject.enabled = true;

				instance.RadioSoundObject.reset();
				instance.RadioSoundObject._play(container);
			}
		}
	}
