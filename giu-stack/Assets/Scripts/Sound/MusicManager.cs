...
public class MusicManager : MonoBehaviour
{
	...
	private void Start()
	{
		...
		PlayMusic(ProfileManager.Instance.EMusic);
		firstRun = false;
		//enebleCounter++;
	...
	//		return;
	//	DebugManager.LogInfo("Music " + type.ToString());
	...
	public void PlayMusic(MusicArrayEnum type)
	{
		...
		if (type == MusicArrayEnum.unknown ||(type == last && IsPlaying))
			return;
		DebugManager.LogInfo("查看当前的  Music " + type.ToString());
		last = type;
		int currentMusic = NowPlays();
		MusicGroup group = GetCurrent(type);
		if (currentG != group && group!=null)
		{
			currentG = group;
			currentMusic = -1;
		}
	...
	MusicGroup GetCurrent(MusicArrayEnum type)
	{
		...
		DebugManager.LogInfo("得到当前的ST value:"+sT);
	...
	void Play()
	{
		if (curSource.isPlaying )
		{
            ...
            to.clip = Resources.Load<AudioClip>("Music/" + current.clip);
			to.volume = 0;
			to.Play();
			fade = eFade.fade;
		...
		{
			curSource = musicSource;
			curSource.clip = Resources.Load<AudioClip>("Music/" + current.clip);
	...
	void Update()
	{
		if(curSource!=null && curSource.clip!=null)
		{
