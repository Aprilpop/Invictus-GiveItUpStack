...
namespace UnityEngine.UI.Extensions
{
    ...
    public class ScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollSnap
    {
        ...
        public void SnapToScreen(int screenNumber)
        {
            UpdateListItemPositions();

            screenNumber = Mathf.Clamp(screenNumber, 0, _pageAnchorPositions.Length - 1);
            _lerp = true;
            _lerpTarget = _pageAnchorPositions[screenNumber];

            PageChanged(screenNumber);
        }
