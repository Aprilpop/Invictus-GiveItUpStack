...
_col =lerp(_col * UNITY_LIGHTMODEL_AMBIENT.rgb, _col * _LightColor0.rgb, diffuseReflection ) ;
...
#ifdef USE_GroundFOGText
	#define FOGG(_col) \
		fixed _dist =distance( _FogCenter.xz , i.posWorld.xz );\
...
*/
		...
		#define BLINK_Frag2(_col, _adj) _col.rgb = lerp(_col.rgb,_ColorS[_toggle].rgb, min(_ColorS[_toggle].a, _adj));	

		#define BLINK_Frag3(_col, _adj) \
		_col.rgb *= _ColorS[_toggle];\
		_col.a *=  min(_ColorS[_toggle].a, _adj);	
